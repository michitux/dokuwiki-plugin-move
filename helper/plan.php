 ...
 */
class helper_plugin_move_plan extends Plugin
{
    /** Number of operations per step  */
    public const OPS_PER_RUN = 10;

    public const TYPE_PAGES = 1;
    public const TYPE_MEDIA = 2;
    public const CLASS_NS   = 4;
    public const CLASS_DOC  = 8;
     ...
     */
    protected $tmpstore = ['pages' => [], 'media' => [], 'ns'    => [], 'affpg' => [], 'miss'  => [], 'miss_media'  => []];
    ...
    public function __construct()
    {
        ...
        $this->files = ['opts'       => $conf['metadir'] . '/__move_opts', 'pagelist'   => $conf['metadir'] . '/__move_pagelist', 'medialist'  => $conf['metadir'] . '/__move_medialist', 'affected'   => $conf['metadir'] . '/__move_affected', 'namespaces' => $conf['metadir'] . '/__move_namespaces', 'missing'    => $conf['metadir'] . '/__move_missing', 'missing_media'    => $conf['metadir'] . '/__move_missing_media'];
    ...
    protected function addMove($src, $dst, $class = self::CLASS_NS, $type = self::TYPE_PAGES)
    {
        ...
        $this->plan[] = ['src'   => $src, 'dst'   => $dst, 'class' => $class, 'type'  => $type];
    }
    ...
    public function commit()
    {
        ...
        usort($this->plan, [$this, 'planSorter']);
    ...
    protected function stepThroughDocuments($type = self::TYPE_PAGES, $skip = false)
    {
        ...
        for ($i = 0; $i < helper_plugin_move_plan::OPS_PER_RUN; $i++) {
            ...
            if ($skip === true) {
                $skip = false;
            } elseif (!$this->MoveOperator->$call($src, $dst)) {
                // move the page
                $log .= $this->build_log_line($mark, $src, $dst, false);
                // FAILURE!
    ...
    protected function findMissingDocuments($src, $dst, $type = self::TYPE_PAGES)
    {
        ...
        foreach ($referenceidx as $page) {
            ...
            } elseif (!file_exists(mediaFN($page))) {
                $newpage = $dst . substr($page, $len);
    ...
    {
        $lists = ['pages' => $this->files['pagelist'], 'media' => $this->files['medialist'], 'ns'    => $this->files['namespaces'], 'affpg' => $this->files['affected'], 'miss'  => $this->files['missing'], 'miss_media'  => $this->files['missing_media']];
